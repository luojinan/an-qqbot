...
async function doFetchToken(
  ...
  console.log(
    `[qqbot-api:${appId}] <<< Status: ${response.status} ${response.statusText}`,
  );
...
export function getTokenStatus(appId: string): {
  ...
  if (tokenFetchPromises.has(appId)) {
    return {
      status: "refreshing",
      expiresAt: tokenCacheMap.get(appId)?.expiresAt ?? null,
    };
  }
...
export async function apiRequest<T = unknown>(
  ...
  const timeout =
    timeoutMs ?? (isFileUpload ? FILE_UPLOAD_TIMEOUT : DEFAULT_API_TIMEOUT);
...
export function startBackgroundTokenRefresh(
  ...
  if (backgroundRefreshControllers.has(appId)) {
    console.log(
      `[qqbot-api:${appId}] Background token refresh already running`,
    );
    return;
  }
  ...
  const refreshLoop = async () => {
    ...
    while (!signal.aborted) {
      try {
        ...
        log?.error?.(
          `[qqbot-api:${appId}] Background token refresh failed: ${err}`,
        );
  ...
  refreshLoop().catch((err) => {
    backgroundRefreshControllers.delete(appId);
    log?.error?.(
      `[qqbot-api:${appId}] Background token refresh crashed: ${err}`,
    );
  });
}
